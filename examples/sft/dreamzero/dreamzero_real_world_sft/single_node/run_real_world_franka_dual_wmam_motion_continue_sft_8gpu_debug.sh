...
export RUN_LOG_PATH=${RUN_LOG_PATH:-/inspire/hdd/project/robot-body/linbokai-CZXS24250037/RLinf/results_franka_dual_wmam_continue_debug}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-real_world_franka_dual_wmam_motion_continue_sft_debug}
...
if [ -n "${RESOLVED_PRETRAINED_MODEL_STATE_DICT_PATH}" ]; then
  echo "[RLinf] Debug continue WMAM from model weights: ${RESOLVED_PRETRAINED_MODEL_STATE_DICT_PATH}"
else
  echo "[RLinf] Debug WMAM starts from configured base Wan2.2/DreamZero weights; init_model_state_dict_path=null."
fi
echo "[RLinf] Fresh optimizer/scheduler/dataloader state; runner.resume_dir is forced to null."

exec "${SCRIPT_DIR}/run_real_world_franka_dual_wmam_motion_sft_8gpu_debug.sh" \
