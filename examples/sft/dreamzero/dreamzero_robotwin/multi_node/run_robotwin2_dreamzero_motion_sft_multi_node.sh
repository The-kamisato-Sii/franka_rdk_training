...
CONFIG=${CONFIG:-robotwin2_motion_sft_dreamzero_5b}
...
start_ray_for_rank() {
  ...
  if [ "${NODE_RANK}" = "0" ]; then
    ...
    echo "[RLinf] Starting Ray worker: node_rank=${NODE_RANK} node_ip=${node_ip} address=${RAY_ADDRESS}"
    local started=0
    for _ in $(seq 1 360); do
      if ray start --address="${RAY_ADDRESS}" --node-ip-address="${node_ip}" "${ray_args[@]}"; then
        started=1
        break
      fi
      sleep 2
