...
# Check whether MASTER_ADDR can be resolved before starting Ray.
if nslookup "${MASTER_ADDR}" > /dev/null 2>&1; then
  echo "域名 [${MASTER_ADDR}] 解析正常"
  IP=$(nslookup "${MASTER_ADDR}" | grep "Address:" | tail -n1 | awk "{print \$2}")
  echo "解析结果: ${IP}"
else
  echo "域名 [${MASTER_ADDR}] 解析失败"
fi
...
CONFIG=${CONFIG:-real_world_joint_sft_dreamzero_motion_sam_scene_flow_5b}
export REAL_WORLD_DATA_ROOT=${REAL_WORLD_DATA_ROOT:-/inspire/qb-ilm2/project/robot-body/public/real_world_data_dreamzero_motion_v2}
export RUN_LOG_PATH=${RUN_LOG_PATH:-/inspire/hdd/project/robot-body/linbokai-CZXS24250037/RLinf/results_fp32}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-real_world_joint_sft_dreamzero_motion_sam_scene_flow_5b_fp32}
