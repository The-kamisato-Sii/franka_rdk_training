...
CONFIG=${CONFIG:-real_world_franka_dual_openpi_pi05_sft_filtered}
export REAL_WORLD_FRANKA_DUAL_ROOT=${REAL_WORLD_FRANKA_DUAL_ROOT:-/inspire/qb-ilm2/project/robot-body/public/bokai/franka_dual_test_filtered}
export RUN_LOG_PATH=${RUN_LOG_PATH:-/inspire/hdd/project/robot-body/linbokai-CZXS24250037/results}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-real_world_franka_dual_openpi_pi05_sft_arrange_vegetables_filtered}
...
export LR=${LR:-1e-5}
export LR_WARMUP_STEPS=${LR_WARMUP_STEPS:-5000}
...
if [ ! -f "${REAL_WORLD_FRANKA_DUAL_ROOT}/stats.json" ] || [ ! -f "${REAL_WORLD_FRANKA_DUAL_ROOT}/norm_stats.json" ]; then
  ...
  echo "[RLinf] Run the TCP filtering command first to build franka_dual_filtered." >&2
  exit 1
fi
