...
export REAL_WORLD_FRANKA_DUAL_ROOT=${REAL_WORLD_FRANKA_DUAL_ROOT:-/inspire/qb-ilm2/project/robot-body/public/bokai/franka_dual_v2}
export RUN_LOG_PATH=${RUN_LOG_PATH:-/inspire/hdd/project/robot-body/linbokai-CZXS24250037/results}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-real_world_franka_dual_openpi_pi05_sft}
...
if [ ! -f "${REAL_WORLD_FRANKA_DUAL_ROOT}/stats.json" ] || [ ! -f "${REAL_WORLD_FRANKA_DUAL_ROOT}/norm_stats.json" ]; then
  ...
  echo "[RLinf] Run: bash toolkits/lerobot/build_franka_dual_test.sh" >&2
  exit 1
fi
